//
// Global state
//
// map     - the map object
// usermark- marks the user's position on the map
// markers - list of markers on the current map (not including the user position)
// 
//

//
// First time run: request current location, with callback to Start
//


if (navigator.geolocation)  {
    navigator.geolocation.getCurrentPosition(Start);
}

function UpdateMapById(id, tag) {

    var target = document.getElementById(id);
    var data = target.innerHTML;

    var rows  = data.split("\n");
   
    for (i in rows) {
	var cols = rows[i].split("\t");
	var lat = cols[0];
	var long = cols[1];

	markers.push(new google.maps.Marker({ map:map,
						    position: new google.maps.LatLng(lat,long),
						    title: tag+"\n"+cols.join("\n")}));
	
    }
}

function ClearMarkers()
{
    // clear the markers
    while (markers.length>0) { 
	markers.pop().setMap(null);
    }
}


function UpdateMap()
{
    var color = document.getElementById("color");
    
    color.innerHTML="<b><blink>Updating Display...</blink></b>";
    color.style.backgroundColor='white';

    ClearMarkers();
    /*
            UpdateMapById("committee_data","COMMITTEE");
            UpdateMapById("candidate_data","CANDIDATE");
            UpdateMapById("individual_data", "INDIVIDUAL");
            UpdateMapById("opinion_data","OPINION");
    */

    if ($('#committee').is(':checked'))
            UpdateMapById("committee_data","COMMITTEE");
    if ($('#candidate').is(':checked'))
            UpdateMapById("candidate_data","CANDIDATE");
    if ($('#individual').is(':checked'))
            UpdateMapById("individual_data", "INDIVIDUAL");
    if ($('#opinion').is(':checked'))
            UpdateMapById("opinion_data","OPINION");


    color.innerHTML="Ready";
   /*
    if (Math.random()>0.5) {
	color.style.backgroundColor='blue';
    } else {
	color.style.backgroundColor='red';
    }
    */
    var count = 0;
    var DEMMoney = 0;
    var REPMoney = 0;


    $("table tbody tr td:nth-child(1)").each(function () {
        count++;
        if ($(this).text() == "DEM") {
            DEMMoney = $("table tbody tr:nth-child(" + count + ") td:nth-child(2)").text();
           // alert("DEM: " + DEMMoney);
        }
        else if ($(this).text() == "REP") {
            REPMoney = $("table tbody tr:nth-child(" + count + ") td:nth-child(2)").text();
            //alert("REP: " + REPMoney);
        }
    });


    if (parseFloat(DEMMoney) > parseFloat(REPMoney)){
        $("#color").css("background-color", "red");
    }
    else if (parseFloat(DEMMoney) < parseFloat(REPMoney)){
        $("#color").css("background-color", "blue");
    }
    else{
        $("#color").css("background-color", "white");
    }

}

function NewData(data)
{
  var target = document.getElementById("data");
  
  target.innerHTML = data;

  UpdateMap();

}


function ViewShift()
{
    var bounds = map.getBounds();

    var ne = bounds.getNorthEast();
    var sw = bounds.getSouthWest();

    var color = document.getElementById("color");

    color.innerHTML="<b><blink>Querying...("+ne.lat()+","+ne.lng()+") to ("+sw.lat()+","+sw.lng()+")</blink></b>";
    color.style.backgroundColor='white';
   
    // debug status flows through by cookie
    var str = "";
    if ($('#committee').is(':checked'))
        str += "committees,";
    if ($('#candidate').is(':checked'))
        str += "candidates,";
    if ($('#individual').is(':checked'))
        str += "individuals,";
    if ($('#opinion').is(':checked'))
        str += "opinions,";

    var select_option = str.slice(0, str.length-1)

    //var cycleYear = "";
 
    str = "";
    $("input[name='cycle']").each(function(){
        if ($(this).is(':checked')){
            str = str + "'" + $(this).attr('id') + "',";
        }
    });
    //alert(str);
    var cycleYear = str.slice(0, str.length-1);
    
   //var cycleYear = "1112";
   
    $.get("rwb.pl?act=near&latne="+ne.lat()+"&longne="+ne.lng()+"&latsw="+sw.lat()+"&longsw="+sw.lng()+"&format=raw&what=" + select_option + "&cycle=" + cycleYear,  NewData);
    $.get("rwb.pl?act=aggregate&latne="+ne.lat()+"&longne="+ne.lng()+"&latsw="+sw.lat()+"&longsw="+sw.lng()+"&format=raw&what=" + select_option + "&cycle=" + cycleYear,  NewDataAggregate);
/*    $.get("rwb.pl?act=near&latne="+ne.lat()+"&longne="+ne.lng()+"&latsw="+sw.lat()+"&longsw="+sw.lng()+"&format=raw&what=committees,candidates", NewData);*/
}


function Reposition(pos)
{
    var lat=pos.coords.latitude;
    var long=pos.coords.longitude;

    map.setCenter(new google.maps.LatLng(lat,long));
    usermark.setPosition(new google.maps.LatLng(lat,long));

    document.cookie = "Lat=" + lat;
    document.cookie = "Long=" + long;
}

function NewDataAggregate(data)
{
    var target = document.getElementById("data_aggregate");

    target.innerHTML = data;

    var count = 0;
    var DEMMoney = 0;
    var REPMoney = 0;

    //alert($("table tbody tr:nth-child(" + count + ") td:nth-child(2)").text());

    $("table tbody tr td:nth-child(1)").each(function () {
        count++;
        if ($(this).text() == "DEM") {
            $(this).parent().css('background-color','red');
            DEMMoney = $("table tbody tr:nth-child(" + count + ") td:nth-child(2)").text();
            //alert("DEM: " + DEMMoney);
        }
        else if ($(this).text() == "REP") {
            $(this).parent().css('background-color','blue');
            REPMoney = $("table tbody tr:nth-child(" + count + ") td:nth-child(2)").text();
            //alert("REP: " + REPMoney);
        }
    });

    if (parseFloat(DEMMoney) > parseFloat(REPMoney)){
        $("#color").css("background-color", "red");
    }
    else if (parseFloat(DEMMoney) < parseFloat(REPMoney)){
        $("#color").css("background-color", "blue");
    }
    else{
        $("#color").css("background-color", "white");
    }


    $("#opinion_aggregate_data tbody tr td:nth-child(1)").each(function () {
        var value = parseFloat($(this).text());
        if (value < 0){
            $(this).parent().css('background-color','red');
            $("#color").css("background-color", "red");

        }
        else if (value > 0){
            $(this).parent().css('background-color','blue');
            $("#color").css("background-color", "blue");
        }
    });

    
}

function Start(location) 
{
  var lat = location.coords.latitude;
  var long = location.coords.longitude;
  var acc = location.coords.accuracy;
  
  var mapc = $( "#map");

  map = new google.maps.Map(mapc[0], 
			    { zoom:16, 
				center:new google.maps.LatLng(lat,long),
				mapTypeId: google.maps.MapTypeId.HYBRID
				} );

  usermark = new google.maps.Marker({ map:map,
					    position: new google.maps.LatLng(lat,long),
					    title: "You are here"});

  markers = new Array;

  document.cookie = "Lat=" + lat;
  document.cookie = "Long=" + long;
  //alert(lat + ", " + long);

  var color = document.getElementById("color");
  color.style.backgroundColor='white';
  color.innerHTML="<b><blink>Waiting for first position</blink></b>";

  google.maps.event.addListener(map,"bounds_changed",ViewShift);
  google.maps.event.addListener(map,"center_changed",ViewShift);
  google.maps.event.addListener(map,"zoom_changed",ViewShift);

  navigator.geolocation.watchPosition(Reposition);

}


