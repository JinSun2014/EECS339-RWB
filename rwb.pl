...
#
#my $dbuser="jsy833";
#my $dbpasswd="zW0sigb5P";

my $dbuser="hmb836";
my $dbpasswd="zMZhcxo04";
...
my $salt = 'ab'; #For Password Encypt
...
if ($action eq "login") { 
  if ($run) { 
    ...
    my $epwd = "";
    if ($user ne "root" && $user ne "anon"){
        $epwd = crypt($password, $salt);}
    else{
        $epwd = $password;}


    #if (ValidUser($user,$password)) { 
    if (ValidUser($user,$epwd)) { 
      ...
      #$outputcookiecontent=join("/",$user,$epwd);
      $action = "base";
      $run = 1;
    } else {
...
if ($action eq "base") { 
  ...
  print "<div>";
  print "<h2>Choose What You Want To See</h2>";
  print "<form id=\"checkform\">";
  print "<input class=\"largerInputBox\" type=\"checkbox\" id=\"committee\" onclick='ViewShift()'><font size=\"8\">Committee</font>";
  print "<input class=\"largerInputBox\" type=\"checkbox\" id=\"candidate\" onclick='ViewShift()'><font size=\"8\">Candidate</font>";
  print "<input class=\"largerInputBox\" type=\"checkbox\" id=\"individual\" onclick='ViewShift()'><font size=\"8\">Individual</font>";
  if (UserCan($user, "query-opinion-data")){
  print "<input class=\"largerInputBox\" type=\"checkbox\" id=\"opinion\" onclick='ViewShift()'><font size=\"8\">Opinion</font>";}
  print "</form>";
  print "</div>";

  #find election cycles

  my @electionCycle;
  eval {@electionCycle = ExecSQL($dbuser, $dbpasswd, "select distinct cycle from cs339.candidate_master");};
  print "<div>";
  print "<h2>Select the cycle</h2>";
  my $i;
  foreach (@electionCycle){
      print "<input class=\"largerInputBox\" type=\"checkbox\" id=@{$_} name=\"cycle\" onclick='ViewShift()'><font size=\"8\">@{$_}</font>";
      #print "<p>@{$_}</p>";
  }
  print "</div>";
  ...
  print "<div id=\"data_aggregate\" style=\:width:100\%; height:10\%\"></div>";
...
=cut
          ...
          eval{
              @temp = ExecSQL($dbuser, $dbpasswd, "select color from rwb_opinions where latitude>? and latitude<? and longitude>? and longitude<?", undef, $latsw, $latne, $longsw, $longne);};
          foreach (@temp) {push(@color, @{$_});}
          my $Color = @color[0];
          print "<div id=\"c\" class=\"$Color\"></div>";
  print "<br><br><br><br><br><br><br><br>aaaaaaaaaaaaaaaaaa<br><br><br><br><br>";
=cut
          print $str;
      }
    }
...
if ($action eq "aggregate") {
    ...
    if ($what{committees} and UserCan($user,"query-fec-data")) {
        my ($str,$error) = Committees_Aggregate($latne,$longne,$latsw,$longsw,$cycle);
        if (!$error) {
            print "<h2>Aggregated committees</h2>$str";
        }
    }
    if ($what{individuals} and UserCan($user,"query-cs-ind-data")) {
        my ($str,$error) = Individuals_Aggregate($latne,$longne,$latsw,$longsw,$cycle);
        if (!$error) {
            print "<h2>Aggregated individuals</h2>$str";
        }
    }
    if ($what{opinions} and UserCan($user,"query-opinion-data")) {
        my ($str,$error) = Opinions_Aggregate($latne,$longne,$latsw,$longsw,$cycle);
        if (!$error) {
            print "<h2>Aggregated opinions</h2>$str";
        }
    }
}
...
if ($action eq "invite-user") { 
    ...
    if (!UserCan($user, "invite-users")){
        print h2('You do not have the required permissions to invite-user');
    } else {
        if (!$run) {
            my @userPerm;
            eval {@userPerm = ExecSQL($dbuser, $dbpasswd, "select distinct action from rwb_permissions where name='$user'");};
            my @temp;
            foreach (@userPerm) {push(@temp, @{$_});}
            #foreach (@userPerm){print "<p>@{$_}</p>";}
            print start_form(-name=>'InviteUser'),
            h2('Invite User'),
                "Email: ", textfield(-name=>'email'),
                p,
                "Please choose the permission you want to give:",
                p,
                checkbox_group(-name=>'perm',-values=>\@temp),
                p,
		      hidden(-name=>'run',-default=>['1']),
            ...
            my @permission = param('perm');
            my @currentTime = localtime(time);
            my $currentTime = join('', @currentTime);
            #send email
            #my $to = $email;
            #my $from = 'do-not-reply@u.northwestern.edu';
            my $subject = "Invite from RWB";
            my $message = "Hi,\n\tThis is an invitation from " .$user;
            $message .= " in RWB. Please click the link below to create a new account.\n ";
            $message .= "\thttp://murphy.wot.eecs.northwestern.edu/~jsy833/rwb/rwb.pl?act=register&refer=$user&perm=".join(',', @permission);
            $message .= "&time=" .$currentTime;
            #checkbox_group(-values=>['a', 'b', 'c', 'd']);
            open(MAIL, "| mail -s $subject $email") or die "Cannot send email\n";
            print MAIL $message;
            close(MAIL);
            eval {ExecSQL($dbuser, $dbpasswd, "insert into link (id, used) values (?,0)", undef, $currentTime);};
            print "<h2>Invite Sent.</h2>";
        }
    }
...
if ($action eq "give-opinion-data") {
    ...
    #print "<p>$inputLat, $inputLong</p>";
    if (!UserCan($user, "give-opinion-data")){
        print h2('you do not have permission give opinion.');
    } else {
        if (!$run){
            print start_form(-name=>'give-opinion'),
            h2('Give Opinion'),
            "Your opinion to this place: ",
            popup_menu(-name=>'myOpinion',
            ...
            my $myOpinion = param('myOpinion');
            my $opinion = 0;
            #if (defined($long) && defined($lat)){print "$long, $lat";}
            #print "$opinion\n";
            if ($myOpinion eq "blue")   {$opinion = 1;}
            elsif ($myOpinion eq "white") {$opinion = 0;}
            elsif ($myOpinion eq "red") {$opinion = -1;}
            else {print "Sorry we cannot accept your opinion.";}
            eval {ExecSQL($dbuser, $dbpasswd, "insert into rwb_opinions (submitter, color, latitude, longitude) values (?,?,?,?)", undef, $user, $opinion, $inputLat, $inputLong);};
            print h2("Your opinion has been successfully added.");
        }
    }
  print "<p><a href=\"rwb.pl?act=base&run=1\">Return</a></p>";
}
...
if ($action eq "register") {
    my @used;
    my $currentTime = param('time');
    eval {@used = ExecSQL($dbuser, $dbpasswd, "select used from link where id=?", undef, $currentTime);};
    #  print "<p>" .join('',@used) ."</p>";
    my @t;
    foreach (@used){
        push(@t, @{$_});
    ...
    if ($t[0] == 1){
        print "<p>This link is already used. Please request for another one.</p>";
    } else {
        if (!$run){
            my $temp = param('perm');

            my $cTime = param('time');
            my @permission = split(/,/, $temp);
            ...
            hidden(-name=>'perms', -default=>$temp),
            hidden(-name=>'time', -default=>$cTime),
            submit,
            end_form,
            hr;
        } else {
            ...
            my $epwd = crypt($password, $salt);
            my $refer = param('refer');
            my $error;
            my $temp = param('perms');
            my @permission = split(/,/, $temp);
            my $crtTime = param('time');
            $error = UserAdd($name, $epwd, $email, $user);
            if ($error) {
                print "Can't register because: $error";
            } else {
                print "Added user $name as referred by $refer\n";

                foreach (@permission){
                    $error = GiveUserPerm($name, $_);
                    if ($error){
                        print "Can't add permission $_ because $error";
                    } else {
                        print "<p>Successfully add permission $_</p>";
                    }
                }
                eval {ExecSQL($dbuser, $dbpasswd, "update link set used=1 where id=?", undef, $crtTime);};
            }
        }
    }
    print "<p><a href=\"rwb.pl?act=base&run=1\">Return</a></p>";
...
sub Committees_Aggregate {
  ...
  eval {
      @rows = ExecSQL($dbuser, $dbpasswd, "
          ...
          sum(cs339.comm_to_cand.TRANSACTION_AMNT + cs339.comm_to_comm.TRANSACTION_AMNT)
          from cs339.committee_master,cs339.cmte_id_to_geo,cs339.comm_to_cand,cs339.comm_to_comm
          where cs339.committee_master.CMTE_ID=cs339.cmte_id_to_geo.CMTE_ID and
          cs339.committee_master.CMTE_ID=cs339.comm_to_cand.CMTE_ID and
          cs339.committee_master.CMTE_ID=cs339.comm_to_comm.CMTE_ID and
          cs339.committee_master.cycle in ($cycle) and latitude>? and latitude<? and longitude>? and longitude<?
          group by cmte_pty_affiliation",
          undef,$latsw,$latne,$longsw,$longne);
  };

    my @temp;
    foreach (@rows){push(@temp, @{$_});}
=cut
    my $i = 0;
    my $REP = 0;
    my $DEM = 0;
    foreach (@temp) {
        $i++;
        if ($_ eq "REP") {$REP = @temp[$i];}
        if ($_ eq "DEM") {$DEM = @temp[$i];}
        print "<p>$_</p>";
    }


    print "<br>-----</br><br>REP--$REP</br><br>DEM--$DEM</br>";
=cut
  if ($@){
      return (undef, $@);
  } else {
      return (MakeTable("Committee_aggregate_data", "2D", ["party","sum"],@rows), $@);
  }

}

sub Individuals_Aggregate {
    my ($latne,$longne,$latsw,$longsw,$cycle) = @_;
    my @rows;
    eval {
        @rows = ExecSQL($dbuser, $dbpasswd, "
            select
            cmte_pty_affiliation,
            sum(transaction_amnt)
            from cs339.individual,cs339.ind_to_geo,cs339.committee_master
            where cs339.individual.SUB_ID=cs339.ind_to_geo.SUB_ID and
            cs339.individual.CMTE_ID=cs339.committee_master.CMTE_ID and
            cs339.committee_master.cycle in ($cycle) and latitude>? and latitude<? and longitude>? and longitude<?
            group by cmte_pty_affiliation",
            undef,$latsw,$latne,$longsw,$longne);
    };
    ...
    if ($@) {
        ...
        return (MakeTable("individual_aggregate_data","2D",
                ["Party", "Sum"],
                @rows),$@);
    }
}

sub Opinions_Aggregate {
    ...
    if ($@) {
        ...
        return (MakeTable("opinion_aggregate_data","2D",
...
sub Committees {
  ...
  my @rows;
  my $command = "select latitude, longitude, cmte_nm, cmte_pty_affiliation, cmte_st1, cmte_st2, cmte_city, cmte_st, cmte_zip ";
  $command .= "from cs339.committee_master natural join cs339.cmte_id_to_geo ";
  $command .= "where cycle in ($cycle) and latitude>? and latitude<? and longitude>? and longitude<?"; 
  eval { 
    @rows = ExecSQL($dbuser, $dbpasswd, $command,undef,$latsw,$latne,$longsw,$longne);
